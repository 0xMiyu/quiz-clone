...
const handler = async (
    ...
    switch (method) {
        case "GET":
            try {
                const questions: QuestionWithOption[] = await questionService.getAllQuestions({quizId})
        ...
        case "POST":
            try {
                const { timeLimit, text, image, points, options }: CreateQuestionRequest = body;
                ...
                const newQuestion: Question = await questionService.createQuestion({quizId, timeLimit, text, image, points, options})
